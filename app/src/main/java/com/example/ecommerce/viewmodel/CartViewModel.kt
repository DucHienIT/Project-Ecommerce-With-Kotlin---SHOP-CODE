...
class CartViewModel(private val cartRepository: CartRepository, private val orderRepository: OrderRepository) : BaseViewModel() {
    ...
    fun orderCart() {
        // Đặt giá trị của đối tượng progressbarLiveData là true để hiển thị tiến trình đang xử lý
        progressbarLiveData.value = true
        orderRepository.order("Bearer ${TokenHolder.access_token}")
            .singleHelper()
            // Không quan tâm đến kết quả trả về khi hành động kết thúc, đặt giá trị của progressbarLiveData là false để ẩn tiến trình
            .doFinally {
                progressbarLiveData.value = false
